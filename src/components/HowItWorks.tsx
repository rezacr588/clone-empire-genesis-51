  ...
  return (
    ...
    >
      <div className="text-center mb-16">
        <p className="text-empire-canyon-deep font-medium mb-3">The Process</p>
        ...
        <div className="relative">
          ...
          {steps.map((step, index) => (
            <div key={index} className="relative flex flex-col md:flex-row gap-8 mb-16 last:mb-0">
              {/* Timeline dot */}
              <div className="absolute left-16 md:left-1/2 w-8 h-8 rounded-full bg-empire-darker border-2 border-empire-canyon-deep transform -translate-x-1/2"></div>
              
              {/* Content side */}
              <div className={`w-full md:w-1/2 ${index % 2 === 0 ? 'md:pr-12' : 'md:pl-12 md:order-1'}`}>
                <div className="bg-empire-darker/50 border border-empire-charcoal/50 rounded-xl p-6 md:p-8 hover:border-empire-canyon-deep/30 transition-all duration-300 h-full">
                  <div className="flex items-center mb-4">
                    <div className={`w-10 h-10 rounded-full flex items-center justify-center ${step.color} bg-empire-darkest mr-4`}>
                      <step.icon size={20} />
                    </div>
                    <span className="text-2xl font-medium text-empire-canyon-deep">{step.number}</span>
                  </div>
